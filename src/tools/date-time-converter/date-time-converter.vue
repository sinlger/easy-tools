...
const loadMarkdown = async () => {
  const mdContent = await import(`./language/date-time-converter.${locale.value}.md?raw`);
...
<template>
  <div>
    <div flex gap-2>
      <c-input-text v-model:value="inputDate" autofocus placeholder="Put your date string here..." clearable
        test-id="date-time-converter-input" :validation="validation" @update:value="onDateInputChanged" />

      <c-select v-model:value="formatIndex" style="flex: 0 0 170px"
